...
df_agg["distance_to_bakhmut"] = haversine2(
    df_agg["latitude"], df_agg["longitude"], bakhmut_coords[0], bakhmut_coords[1]
)
df_agg["distance_to_soledar"] = haversine2(
    df_agg["latitude"], df_agg["longitude"], soledar_coords[0], soledar_coords[1]
)
df_agg["distance_to_avdiivka"] = haversine2(
    df_agg["latitude"], df_agg["longitude"], avdiivka_coords[0], avdiivka_coords[1]
)
df_agg["distance_to_vuhledar"] = haversine2(
    df_agg["latitude"], df_agg["longitude"], vuhledar_coords[0], vuhledar_coords[1]
)
df_agg["distance_to_robotyne"] = haversine2(
    df_agg["latitude"], df_agg["longitude"], robotyne_coords[0], robotyne_coords[1]
)
df_agg["distance_to_kupiansk"] = haversine2(
    df_agg["latitude"], df_agg["longitude"], kupiansk_coords[0], kupiansk_coords[1]
)


df_agg["min_distance_to_battlefront"] = df_agg[
    [
        "distance_to_bakhmut",
        "distance_to_soledar",
...
df_agg["civilian_targeting_encoded"] = df_agg["civilian_targeting"].apply(
...
df_agg["event_explosions"] = (
    (df_agg["event_type"] == "Explosions/Remote violence")
    & (df_agg["civilian_targeting_encoded"] == 0)
).astype(int)
df_agg["event_explosions_civilians"] = (
    (df_agg["event_type"] == "Explosions/Remote violence")
    & (df_agg["civilian_targeting_encoded"] == 1)
).astype(int)
df_agg["event_violence_civilians"] = (
    df_agg["event_type"] == "Violence against civilians"
).astype(int)
...
df_agg["distance_to_densest"] = np.sqrt(
    (df_agg["tsne_0"] - densest_point[0]) ** 2
    + (df_agg["tsne_1"] - densest_point[1]) ** 2
)

# Convert into a distance score
decay_factor = 0.05  # This is just an example value; adjust as needed
df_agg["tsne_distance_points"] = 50 * np.exp(
    -decay_factor * df_agg["distance_to_densest"]
)

# Compute physical distance score based on min_distance_to_battlefront:
df_agg["distance_points"] = 100 * np.exp(
    -decay_factor * df_agg["min_distance_to_battlefront"]
)
...
df_agg["hazard_score"] = (df_agg["distance_points"] * weight_distance) + (
    df_agg["tsne_distance_points"] * weight_tsne
)
...
df_agg["hazard_score"] = (
    (df_agg["hazard_score"] - min_hazard) / (max_hazard - min_hazard)
) * 100
...
high_hazard_locs = df_agg[df_agg["hazard_score"] >= 80][
...
df_agg["hazard_score"] = df_agg.apply(
